...
from unified import fit_multiprocess, calculate_membership_mat
from utils import read_data, plot_centroids_multi, split_in_partitions
import numpy as np
...
if __name__ == '__main__':
    ...
    if show_accuracy is True:
        ...
        lst_cnt = list((cnt0, cnt1, cnt2))
        lst_lastDict = list((lastDict[0], lastDict[1], lastDict[2]))
        lst_cnt.sort()
        lst_lastDict.sort()
        precision_0 = lst_lastDict[0] / lst_cnt[0]
        precision_1 = lst_lastDict[1] / lst_cnt[1]
        precision_2 = lst_lastDict[2] / lst_cnt[2]
        print("precisions:", str(lst_lastDict[0]) + '/' + str(lst_cnt[0]), str(lst_lastDict[1]) + '/' + str(lst_cnt[1]),
              str(lst_lastDict[2]) + '/' + str(lst_cnt[2]))
        ...
        recall_0 = lst_lastDict[0] / (np_data.shape[0] / 3)
        recall_1 = lst_lastDict[1] / (np_data.shape[0] / 3)
        recall_2 = lst_lastDict[2] / (np_data.shape[0] / 3)
        recall_total = (lst_lastDict[0] + lst_lastDict[1] + lst_lastDict[2]) / np_data.shape[0]
        print("recalls:", str(lst_lastDict[0]) + '/' + str(np_data.shape[0] / 3),
              str(lst_lastDict[1]) + '/' + str(np_data.shape[0] / 3),
              str(lst_lastDict[2]) + '/' + str(np_data.shape[0] / 3))
        print("recalls:", recall_0, recall_1, recall_2)
        print("recall total:", recall_total)
    plot_centroids_multi(data, centroids)
